...
namespace MinimalChess
{
    ...
    public class Board
    {
        ...
        private void AddCaptures(Action<Move> moveHandler, int square, byte[][] targets)
        {
            ...
            {
                // This check is correct from the previous fix
                if (IsOpponentPiece(_state[target]) && (_state[target] & Piece.TypeMask) != Piece.King)
        ...
        private void AddCaptures(Action<Move> moveHandler, int square, byte[][][] targets)
        {
            foreach (var axis in targets[square])
            {
                foreach (int target in axis)
                {
                    if (_state[target] != Piece.None)
                    {
                        // CRITICAL FIX: Apply the same king-capture check to this method as well.
                        if (IsOpponentPiece(_state[target]) && (_state[target] & Piece.TypeMask) != Piece.King)
                            AddMove(moveHandler, square, target);

                        // Stop searching this direction regardless of what the piece was.
                        break;
                    }
                }
            }
