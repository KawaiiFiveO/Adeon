...
namespace MinimalChessEngine
{
    ...
    {
        const string NAME_VERSION = "Adeon Chess Engine";
        const string AUTHOR = "OneHalf (with libraries by Thomas Jahn, algorithms by TICT-HQ)";
        ...
        {
            if (tokens.Length > 4 && tokens[1] == "name" && tokens[2] == "Hash" && tokens[3] == "value" && int.TryParse(tokens[4], out int hashSizeMBytes))
            {
        ...
        private static void UciGo(string[] tokens)
        {
            ...
            TryParse(tokens, "depth", out int maxDepth, 64);
